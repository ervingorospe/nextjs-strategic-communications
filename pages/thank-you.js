...
export default function Thankyou({ collections, seo }) {
  ...
  return (
    <div className="w-full">
      <Head>
        <title>Thank You - {metaTitle}</title>
        <link rel="canonical" href={hostname}/>
        <meta name="title" content={`Thank You - ${metaTitle}`} />
        <meta name="description" content={`${_.get(metas, 'metaDescription')}`}/>
        <meta property="og:title" content={`Thank You - ${metaTitle}`} key="title" />
...
export async function getStaticProps(context) { 
  const pageNavigation = await getItem('153217')
