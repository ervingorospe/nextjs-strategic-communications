...
export default function About({ collections, seo }) {
  ...
  return (
    <div className="w-full">
      <Head>
        ...
        <meta name="title" content={`About - ${metaTitle}`} />
        <meta name="description" content={`${_.get(metas, 'metaDescription')}`}/>
        <meta property="og:title" content={`About - ${metaTitle}`} key="title" />
...
export async function getStaticProps(context) { 
  const pageNavigation = await getItem('153226')
