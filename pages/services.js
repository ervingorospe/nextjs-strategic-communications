...
export default function Services({ collections, seo }) {
  ...
  return (
    <div className="w-full">
      <Head>
        ...
        <meta name="title" content={`Services - ${metaTitle}`} />
        <meta name="description" content={`${_.get(metas, 'metaDescription')}`}/>
        <meta property="og:title" content={`Services - ${metaTitle}`} key="title" />
...
export async function getStaticProps(context) { 
  const pageNavigation = await getItem('153224')
