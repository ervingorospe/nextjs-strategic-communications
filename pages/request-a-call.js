...
export default function RequestACall({ collections, seo }) {
  ...
  return (
    <div className="w-full">
      <Head>
        <title>Request A Call - {metaTitle}</title>
        <link rel="canonical" href={hostname}/>
        <meta name="title" content={`Request A Call - ${metaTitle}`} />
        <meta name="description" content={`${_.get(metas, 'metaDescription')}`}/>
        <meta property="og:title" content={`Request A Call - ${metaTitle}`} key="title" />
...
export async function getStaticProps(context) { 
  const pageNavigation = await getItem('153676')
