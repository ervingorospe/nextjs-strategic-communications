...
export default function Contact({ collections, seo }) {
  ...
  return (
    <div className="w-full">
      <Head>
        ...
        <meta name="title" content={`Contact - ${metaTitle}`} />
        <meta name="description" content={`${_.get(metas, 'metaDescription')}`}/>
        <meta property="og:title" content={`Contact - ${metaTitle}`} key="title" />
...
export async function getStaticProps(context) { 
  const pageNavigation = await getItem('153223')
